...
After reading the proposed exercise I usually start by writing or drawing what I think are the relevant elements involved in the problem, so, I have identified the following facts:
...
- we need to perform some calculation for each order so we can know what is the disbursement
- we would like to use any kind of background job to regularly filter orders and extract our fee per merchant for such period (weekly)
...
- calculating total disbursements per merchant depends on the date that order went completed. Disbursements are per week and are calculated weekly
...
- when retrieving orders to calculate disbursements we should only use those order with `completed` as `true` and week number and year same as specified period
- we can have a table for disbursements so we have the merchant, the disbursement, the starting date and ending date for the week or the week number and year, so we have 52 rows per merchant per year, tops.
...
- disbursement (will point to `Disbursement` model )
- amount
- created_at
- completed_at
...
Those are the models inferred from the data, however I think I will use another model to store disbursements, so it's pretty easy to query it, adding only 52 rows per year per merchant, so queries will be lightning fast compared to a query to `Order` model filtering by completed, week number, year and merchant.
...
- id is merchant id, optional
- date is any given date to be used to calculate week, optionalm if not provided previous week to current date will be used
...
Download the repo to the machine you want to use. You'll find one main folder, `sequra_api`, a `docker-compose.yml` file, a `Makefile` file and this [`README.md`](http://readme.md) file.
